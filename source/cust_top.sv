...
//    output  logic[1:0]      pmod_ss
    ...
    assign max_cnvtb       = 1'b1; // converts on spi command
    
//    // make tec_ref_clk at 100MHz
//    logic clk, tec_ref_clk_int;
//    ODDR #(.DDR_CLK_EDGE("SAME_EDGE"), .INIT(1'b0), .SRTYPE("SYNC")) tec_ref_ODDR (.Q(tec_ref_clk_int), .C(clk), .CE(1'b1), .D1(1'b1), .D2(1'b0), .R(1'b0), .S(1'b0));
//    OBUF tec_ref_OBUF (.O(tec_ref_clk), .I(tec_ref_clk_int));

    logic[31:0] count = 0;
    always_ff @(posedge clk) begin
        count <= count + 1;
        tec_ref_clk <= count[7]; // divide by 256 = 390KHz 
    end
